...
export async function listAllBrands(first = 100, after = "") {
  try {
    const data = await fetchAPI(
      `
      query{
        allBrands(first: ${first}, after: "${after}"){
            edges{
              node{
                brand_name
                _meta{
    ...
    return data?.allBrands?.edges && data.allBrands.edges.length > 0
      ? {
          brands: data.allBrands.edges,
          pageInfo: {
            ...data.allBrands.pageInfo,
          },
        }
      : {};
  } catch (error) {
    console.log("List brands error:", error);
  }
}
...
export async function getBrandByUid(uid) {
  try {
    ...
    console.log("Get brands by uid error:", error);
  }
}
...
export async function getBrandRangeByUid(uid) {
  try {
    ...
    console.log("Get brands range by uid error:", error);
  }
}
...
export async function getVehiclesDirectory() {
  try {
    const data = await fetchAPI(
      `
      query {
        allVehicle_directorys {
          edges {
            node {
              page_heading_1
              page_heading_2
    ...
    return data?.allVehicle_directorys?.edges &&
      data.allVehicle_directorys.edges.length > 0
      ? data.allVehicle_directorys.edges[0].node
      : {};
  } catch (error) {
    console.error("Get Brand categories error:", error);
  }
...
export async function getAllVehicles(first = 100, after = "") {
  try {
    const data = await fetchAPI(
      `
      query {
    ...
    return data?.allVehicles?.edges && data.allVehicles.edges.length > 0
      ? {
          vehicles: data.allVehicles.edges,
          pageInfo: {
            ...data.allVehicles.pageInfo,
          },
        }
      : {};
  } catch (error) {
...
export async function getVehicleByUid(uid) {
  try {
    const data = await fetchAPI(
      `
      query{
...
export async function getVehicleCollectionByUid(uid) {
  try {
    const data = await fetchAPI(
      `
      query{
        allVehicle_collections(uid:"${uid}"){
          edges{
            node{
              page_heading_1
              page_heading_2
    ...
    return data?.allVehicle_collections?.edges &&
      data.allVehicle_collections.edges.length > 0
      ? data.allVehicle_collections.edges[0].node
      : {};
  } catch (error) {
    console.error("Get Vehicle collection error:", error);
  }
}

export async function getVehicleCategoryByUid(uid) {
  try {
    const data = await fetchAPI(
      `
      query{
        allVehicle_categorys(uid:"${uid}"){
          edges{
            node{
              page_heading_1
              page_heading_2
    ...
    return data?.allVehicle_categorys?.edges &&
      data.allVehicle_categorys.edges.length > 0
      ? data.allVehicle_categorys.edges[0].node
      : {};
  } catch (error) {
